...
在实现层面，UID 需要同时满足两个目标：
1. **避免误点**：当页面发生变化导致旧 UID 失效时，应明确报错而不是“点到别的元素”。因此 UID 不能被随意复用到其他节点。
2. **降低抖动**：在同一页面会话中，若元素的 `backendNodeId` 没有变化，系统会尽量复用已分配的 UID，使连续快照中的 UID 更稳定，便于重试与恢复流程。

需要注意：页面刷新、路由切换、框架重渲染可能导致 `backendNodeId` 变化，此时 UID 仍可能改变，这是浏览器底层节点标识变化所致。
